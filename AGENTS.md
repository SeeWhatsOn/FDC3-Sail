...
- `@finos/sail-desktop-agent` integrator docs use progressive disclosure: `README.md` stays Level 0–1 (minimal role-first ASCII; npm package pages do not render Mermaid); full runtime layering, host-contract examples, deployment fork, and Mermaid diagrams live in `packages/sail-desktop-agent/docs/browser-edge-and-da.md`, with WCP/DACP terminology only in advanced sections.
