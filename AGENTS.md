...
| Tag | Use |
|-----|-----|
| `@conformance2.2` | Scenario maps to the FDC3 2.2 conformance / interop test pack for that API area. |
| `@conformance3.0` | Scenario maps to the FDC3 v3.0 conformance pack (e.g. `fdc3.close()` / `closeRequest`). Use for v3.0-only APIs — not `@conformance2.2`. |
...
- `@finos/sail-desktop-agent` exposes manual composition (transports, host contracts, app-connection mechanisms) and high-level presets; application code imports `createBrowserDesktopAgent` from `@finos/sail-desktop-agent/presets` (top-level re-exports the same factory for convenience). Browser-first preset couples an in-tab `DesktopAgent` to WCP through a DA-owned app connection router; apps still use WCP plus per-app `MessagePort`, but integrators do not choose or deploy a browser-path `Transport`, `BrowserDaEdgeLink`, or remote DA client. Public `createWCPClient` remote-DA preset was removed (BFDA-02) — no supported server/worker-hosted DA adoption path on `v3-pre`. Convenience factories (`createBrowserDesktopAgent`, session/intent-resolver wiring) belong in `presets/`; `app-connection/` is mechanisms only (`WCPConnector`, `MessagePortTransport`, DA-owned app connection router, WCP protocol). Presets abstract common wiring but must not hide the composition path — returns a coupled `DesktopAgent` (browser app connection started/stopped with the agent) with optional `onAppConnected`/`onAppDisconnected`/`onHandshakeFailed` for host lifecycle without exposing `wcpConnector`; handler-only tests may still use `new DesktopAgent({ transport })` or `MockTransport`. For browser preset host UI evolution, prefer lower-case grouped returned control surfaces (`intentResolver`, `channels`, `apps`) with subscription + command methods; keep `appLauncher` as a host-provided option callback, not a returned `AppLauncher` surface. Browser preset `apps` controller: ship `disconnect(instanceId)` for host teardown; do not add `apps.close()` — FDC3 v3.0 `fdc3.close()` stays app-initiated via optional `AppLauncher.close`, not a grouped controller method (defer host-initiated close/teardown spike).
- Keep `@finos/sail-desktop-agent` aligned with FDC3 2.2 spec behavior; Sail-specific extensions (e.g. WCP origin allowlists) belong in `@finos/sail-platform-api`, not the core library. Prepare FDC3 3.0 incrementally on one DACP handler tree (optional wire fields, `@conformance3.0` BDD); do not split `handlers/v2` vs `v3`. npm package `3.0.0-pre.x` semver ≠ `getInfo().fdc3Version` (`"2.2"` until 3.0 conformance lands).
- FDC3-Sail product defaults live in `packages/sail-desktop-agent/src/core/sail-default-config.ts`; `new DesktopAgent(options)` deep-merges them in the constructor (no handler-level `??` fallbacks). `resolveDesktopAgentConfig()` stays exported for tests; app code uses `new DesktopAgent({ transport, ... })`, `createBrowserDesktopAgent()`, or `SailPlatform`. Product identity (`provider`, `providerVersion`, `FDC3-Sail` branding) stays consistent across library and platform; `providerVersion` tracks `@finos/sail-desktop-agent` npm semver via TypeScript factory/config APIs, not env vars or JSON/YAML files.
- Keep `@finos/sail-desktop-agent` headless but usable on its own: FDC3 host contracts (`AppLauncher`, `IntentResolver`, `ChannelControl`, app directory, lifecycle events) belong in the core package; `@finos/sail-platform-api` implements those contracts and adds workspace, layout, config, and product shell — pure DA adopters must not need platform-api. Intent resolver UI is for DA/platform builders, not FDC3 app developers: preserve app-facing FDC3 2.2 behavior, bypass resolver UI for explicit `AppIdentifier` and unambiguous matches, and expose browser-host ergonomics as framework-agnostic `onRequest`/`select`/`cancel` style helpers over a host-control transport rather than React- or WCPConnector-specific UI APIs. Official DACP/WCP wire types use `BrowserTypes` from `@finos/fdc3`; deep `@finos/fdc3-schema/dist/generated/api/BrowserTypes` only for runtime validators. Sail host UI adapter types (`HostIntentResolver*`) belong in `host-contracts/`; Sail-internal DACP resolver callbacks in `core/handlers/dacp/intent-resolution-callback.ts` — not in `protocols/wcp/` or generic handler `types.ts`. Do not route host shell UI through `Fdc3UserInterfaceResolve` (injected-iframe flow); channel selector UI should follow the same host-contract controller pattern.
...
- Cucumber `@conformance2.2` (~103 scenarios) uses MockTransport with pre-registered instance ids; green BDD does not prove browser WCP. Only `wcp-desktop-agent.integration.test.ts` exercises the WCP↔DA seam today; toolbox `AppTimeout` clusters need WCP multi-app delivery tests and harness coverage, not more MockTransport channel BDD alone. `CustomWorld.initializeDesktopAgent()` defaults `heartbeatEnabled: false` — heartbeat scenarios opt in via `A desktop agent with heartbeat checking`. Do not wire `requestIntentResolution` on default init; lazy-wire for resolver UI/cancel scenarios. Directory apps in BDD use production WCP4→WCP5; apps without app-directory entries may use direct CONNECTED.
...
- This monorepo uses npm workspaces (`npm install`, `npm test`), not pnpm; always install from the repo root — shared dev tooling (TypeScript, Vite, Vitest, ESLint, Prettier, React types) is hoisted in root `package.json`; workspace packages keep only package-specific devDependencies (Cucumber, Playwright); run workspace scripts with `npm run <script> -w <package>`. Vite+ (`vite-plus` local devDependency) is the planned toolchain consolidation — npm scripts are the contributor interface (`npm run check`, `npm run test`, etc.); do not require or document global `vp` CLI install (fin-services constraint). Full Vite+ migration replaces Husky+lint-staged with `vp config` + `staged` in `vite.config.ts` when landed.
...
- `@finos/sail-desktop-agent` `src/` layout is role-first at the top (`core/`, `app-connection/`, `host-contracts/`, `presets/`, `transports/`) with FDC3 spec detail in subfolders (`core/dacp/`, `app-connection/wcp/`; `connectors/browser/` → `app-connection/`, `protocols/*` folded under those). `host-contracts/` and `presets/` stay separate top-level folders; all convenience factories live in `presets/`, not `app-connection/`. `Transport` interface stays in `core/interfaces/` for DA core and handler-only tests; environment-agnostic implementations live in `transports/`, while browser-first app delivery uses WCP plus per-app `MessagePort` through the DA-owned app connection router. Shared Vitest harness and package boundary tests stay in `src/__tests__/`. App catalog: launchable apps live on `AgentState.appDirectory`; reads via `app-directory-queries.ts` / `fetch-app-directory.ts`, writes via `state/mutators/app-directory.ts` — no `AppDirectoryManager` or `DesktopAgent.getAppDirectory()`; Cucumber seeds via `config.apps`. Browser FDC3 stack (integrator guide: `website/docs/packages/desktop-agent/integrator-guide.md`): `createBrowserDesktopAgent` returns a coupled `DesktopAgent`; `DesktopAgent` owns DA state plus starts/stops its DA-owned WCP app connection; `WCPConnector` remains the WCP1–3 and MessagePort routing mechanism behind that preset, not a separate host-facing session. `getBrowserDesktopAgentSession` throws for collapsed browser agents except legacy in-repo test access; use grouped host controllers and `DesktopAgent.getAppConnection()` / `getAppConnections()` for browser app connection metadata. DACP handlers deliver responses through `DacpResponseDispatcher` on `DACPHandlerContext.responses` (`sendToInstance` / `sendOutbound`), not raw `context.transport`. Intent resolution is one host-global flow per DA (`requestId`-correlated); the current host contract is `IntentResolver.resolve(request)`, while FDC3 DACP also defines `Fdc3UserInterfaceResolve` / `Fdc3UserInterfaceResolveAction` for injected resolver UIs. Treat `WCPConnector.on("intentResolverNeeded")` as an internal browser app-connection adapter, not the canonical UI state API — normal host UI uses grouped `channels` / `intentResolver` / `apps` controllers. Channel membership is per connected `instanceId`, not `appId`. User channels are fixed at DA construction — no public runtime add-channel API; `ChannelControl` is a documented contract shape, not a preset wiring option yet. InMemoryTransport is not the toolbox failure surface — `AppTimeout` points at MessagePort/instanceId routing. DACP handlers use `resolveDacpHandlerInstanceId` (`utils/resolve-context-listener-instance-id.ts`): prefer the MessagePort-routed instance when already registered before redirecting to a pending open-with-context host id — otherwise `app-control` `closeWindow` broadcasts miss mock apps and orphan tabs accumulate.
- WCP4 over `InMemoryTransport` (browser preset, conformance harness): do not put `Window` on DACP message meta — `InMemoryTransport.send` uses `structuredClone` and throws `DataCloneError`; store WCP1Hello source windows in `wcp-pending-source-window.ts` keyed by temp `instanceId` and resolve in `wcp-handlers.ts`. `raiseIntentResultResponse` must not share the same object reference in `payload.metadata` and `intentResult.metadata` — `structuredClone` treats shared refs as cycles and breaks WCP intent-result delivery (use separate clones, e.g. `cloneIntentResultContextMetadata`).
