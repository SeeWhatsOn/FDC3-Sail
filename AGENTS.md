...
- `@finos/sail-desktop-agent` exposes manual composition (transports, host contracts, app-connection mechanisms) and high-level presets; application code imports `createBrowserDesktopAgent` from `@finos/sail-desktop-agent/presets` (top-level re-exports the same factory for convenience). Browser-first preset couples in-tab `DesktopAgent` + WCP via `BrowserDaEdgeLink` (`createBrowserDesktopAgentEdgeLink()`); public `createWCPClient` remote-DA preset was removed (BFDA-02) — no supported server/worker-hosted DA adoption path on `v3-pre`. Convenience factories (`createBrowserDesktopAgent`, session/intent-resolver wiring) belong in `presets/`; `app-connection/` is mechanisms only (`WCPConnector`, `MessagePortTransport`, `BrowserDaEdgeLink`, WCP protocol). Presets abstract common wiring but must not hide the composition path — returns a coupled `DesktopAgent` (browser edge started/stopped with the agent) with optional `onAppConnected`/`onAppDisconnected`/`onHandshakeFailed` for host lifecycle without exposing `wcpConnector`; manual wiring uses `WCPConnector` + edge link or `new DesktopAgent({ transport })` for tests. For browser preset host UI evolution, prefer lower-case grouped returned control surfaces (`intentResolver`, `channels`, `apps`) with subscription + command methods; keep `appLauncher` as a host-provided option callback, not a returned `AppLauncher` surface. Browser preset `apps` controller: ship `disconnect(instanceId)` for host teardown; do not add `apps.close()` — FDC3 v3.0 `fdc3.close()` stays app-initiated via optional `AppLauncher.close`, not a grouped controller method (defer host-initiated close/teardown spike).
