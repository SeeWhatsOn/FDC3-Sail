...
- **WCP4 temp vs WCP5 canonical ids:** During identity validation, DACP handler `context.instanceId` may still be the temp connection id while heartbeat and cleanup use the canonical WCP5 `instanceId`. Single contract: `protocols/wcp/wcp-instance-id-resolver.ts` (`linkTempToCanonical`, `resolveCanonicalInstanceId`, `unlinkCanonical`). Link at WCP5 success in `wcp-handlers.ts` regardless of `heartbeatEnabled`; `MockTransport.registerWcp5Mapping` delegates to the same module. Cleanup and listener routing resolve via `resolveCleanupInstanceId` in `cleanup.ts`. Browser preset wraps `disconnectInstance` to tear down WCP connector after DA cleanup. Cucumber: register `connectionId → lastWcp5ValidatedInstanceId` on `MockTransport` after WCP validate; use canonical id for liveness, goodbye, and disconnect steps.
...
- **App directory catalog** lives on `AgentState.appDirectory` (`apps`, `directoryUrls`); use exported query functions (`app-directory-queries.ts`) and Immer mutators (`state/mutators/app-directory.ts`). `AppDirectoryManager` is removed — seed via `DesktopAgent` config `apps` or mutators; remote load via `loadDirectory()` / `replaceDirectoryUrls()`.
- **`AppInstanceState` is `PENDING` | `CONNECTED` only** — disconnect removes the instance record (`removeInstance`); no `TERMINATED`/`intentListeners` on instances; intent discovery uses `state.intents.listeners` only.
- **User channel runtime SSOT:** `state.channels.user` seeded once at construction; host/handler reads use `getAllUserChannels(state)` — not a separate config cache on `DesktopAgent`.
...
- Avoid `import ... with { type: "json" }` in this repo: the TypeScript parser treats `with` as a legacy statement and breaks module parsing (cascading false module-not-found and `error`-typed imports). Use plain `import pkg from "../../package.json"` with `resolveJsonModule`, or `readFileSync(new URL(..., import.meta.url))` in tests. For DACP test message metadata, use `BrowserTypes.AppRequestMessageMeta`; `BrowserTypes.RequestMessage["meta"]` is not a valid `@finos/fdc3` type and resolves unsafely.
