...
- On `@finos/sail-desktop-agent` v3 refactor work, backward-compatibility shims are not required — delete legacy classes/facades (e.g. removed `AppDirectoryManager`, `getAppDirectory()`) and `@deprecated` re-exports; import from canonical paths (e.g. `host-contracts/`, `app-directory-queries.ts`).
