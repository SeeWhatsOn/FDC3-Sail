...
- `@finos/sail-conformance-harness` is the clean-room FDC3 toolbox host (port 3001); targets FDC3 3.0 via `HARNESS_FDC3_TARGET_VERSION` in `harness-bootstrap.ts` (`sail-web` and DA `default-config.ts` still `"2.2"`). Compare against full Sail stack (`sail-web`, port 3000). Harness dev: `npm run dev -w @finos/sail-conformance-harness` runs `tsdown --watch` for `@finos/sail-desktop-agent` concurrently with Vite — no separate DA build during harness work; Vite picks up `node_modules/@finos/sail-desktop-agent/dist` changes. FINOS Mocha runs inside Conformance1 iframe (`fdc3-compliance.js` from hosted toolbox); the harness only boots DA, launcher, and browsing contexts — triage failures as host teardown, WCP/open-with-context delivery, or FDC3 client proxy APIs (`getResultMetadata`, `AppMetadata.desktopAgent`), not DA version gating alone. Layout: Conformance1 iframe; mock apps with `hostManifests.sail.forceNewWindow` open in popup tabs. Instance lifecycle: pre-register host `instanceId` before WCP4 (`registerPendingHostInstance`); disconnect on popup close/WCP6. FINOS scenario teardown: Conformance1 broadcasts `closeWindow` on `app-control` → mock broadcasts `windowClosed`/`fdc3.nothing` → mock calls `fdc3.close()`; `harness-finos-teardown.ts` disconnects mock instances via `onInboundAppMessage` on `BrowserAppConnection`; `collectHarnessCloseInstanceIds` must not treat unrelated connections as close targets. FDC3 v3.0 `closeResponse` `ApiTimeout` is expected when the app is destroyed. App directory: `conformance-appd.json` (hosted URLs) loaded through `conformance-app-directory.ts` — `VITE_CONFORMANCE_TOOLBOX=local` rewrites `https://fdc3.finos.org/toolbox/fdc3-conformance` → `http://localhost:3001` (paths after base unchanged; local profile → FDC3 2.2, hosted default → 3.0); never merge two full conformance catalogs (same 18 `appId`s conflict). Harness disables channel selector UI. Toolbox exports live under `packages/sail-conformance-harness/results/`. Default Sail web demo app directory: `packages/sail-web/fixtures/default-app-directory.json`. Hosted FINOS `(GetAgentAPI) Method is callable` still asserts `getInfo().fdc3Version.startsWith("2.")` — may fail when harness advertises `"3.0"` (stale toolbox oracle). `(GetInfo2)` is open-with-context + popup MetadataApp + app-control, not standalone `getInfo`. Open-with-context 20s Mocha hangs mean `open()` never settled (pending lifecycle), not 15s `AppTimeout`. `findIntent` `apps.length === 1` needs session hygiene after incomplete teardown, not naive dedupe.
