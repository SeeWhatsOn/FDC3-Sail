...
| BHA-04 | App directory state can be mutated internally but runtime host catalog registration is not exposed as a grouped browser API. | **done** — work item deleted (2026-06-20) | Full `desktopAgent.apps` controller | `add-browser-apps-controller` |
...
| `add-browser-apps-controller` | done — work item deleted | 2026-06-20 — `desktopAgent.apps` catalog, open, lifecycle, disconnect |
