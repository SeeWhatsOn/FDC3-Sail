...
- Phase A (test-engineer): RED — 5 Vitest + 2 Cucumber failures (wire vs client metadata gap).
- Phase B (implement-agent): GREEN — attach `intentResult.metadata`, intent raise app metadata merge; 308 Vitest + 4 Cucumber intent-result pass.
- Phase C/D: **blocked** — verifier-agent / code-reviewer hit session usage limit; orchestrator re-ran tests locally (exit 0).

## Staged for review

**Scope:** `fix-toolbox-metadata-client-and-dacp-paths`

**Key changes:**
- `intent-result-metadata.ts` — client metadata attach + ContextWithMetadata wire shape
- `intent-result-handlers.ts` — `intentResult.metadata` for `getResultMetadata()`
- `intent-delivery-helpers.ts` — forward app traceId/signature/antiReplay/custom on intent raise
- New Vitest: `intent-result-client-metadata`, `intent-context-metadata-on-raise`, `get-app-metadata-harness-path`
- Cucumber: `intentResult.metadata.traceId` columns; `testing-utils` `MUST-BE-NON-EMPTY`

**Tests (local):**
```bash
npm test -w @finos/sail-desktop-agent -- intent-result-client-metadata intent-context-metadata-on-raise get-app-metadata-harness-path intent-result-handlers intent-result-metadata  # exit 0
npx cucumber-js --profile single test/features/intents/intent-result.feature  # 4/4 pass
```
