...
async function getCompanyNews(symbol: string, from: string, to: string): Promise<NewsItem[]> {
  try {
    ...
    if (!apiKey) {
      console.warn("VITE_FINNHUB_API_KEY is not set. News feed will not work without an API key.")
      return []
    }

    console.log(`Fetching news for ${symbol} from ${from} to ${to}`)
    const news_url = `https://finnhub.io/api/v1/company-news?symbol=${symbol}&from=${from}&to=${to}&token=${apiKey}`
    ...
    if (!Array.isArray(data)) {
      console.error("Finnhub API returned non-array response:", data)
      return []
    }
            ...
            <div className="col-span-2 text-center py-10">
              <p className="text-xl text-gray-400">
                {!import.meta.env.VITE_FINNHUB_API_KEY
                  ? "Finnhub API key not configured. Please set VITE_FINNHUB_API_KEY environment variable."
