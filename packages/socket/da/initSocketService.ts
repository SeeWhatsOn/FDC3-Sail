...
export function initSocketService(
  ...
  io.on("connection", (socket: Socket) => {
    ...
    // Handles initial connection from an Electron instance, either returning app details or creating a new Desktop Agent session
    socket.on(
      ELECTRON_HELLO,
      function (
    ...
    // Handles connection from a Desktop Agent, creating or updating a session
    socket.on(
      DA_HELLO,
      function (
    ...
    // Provides the list of available apps from the directory to the Desktop Agent
    socket.on(
      DA_DIRECTORY_LISTING,
      async function (
    ...
    // Registers a new app launch, generating a unique instance ID and preparing for connection
    socket.on(
      DA_REGISTER_APP_LAUNCH,
      async function (
    ...
    // Updates server-side state with client state information, including app directories, channels, and panel configurations
    socket.on(
      SAIL_CLIENT_STATE,
      async function (
        ...
        const session = await getFdc3ServerInstance(
          sessions,
          props.userSessionId,
        )
        if (session) {
          ...
          props.panels.forEach((panel) => {
            ...
            const state = session.serverContext.getInstanceDetails(
              panel.panelId,
            )
            if (state) {
              ...
              if (newChannel !== existingChannel) {
                session.serverContext.notifyUserChannelsChanged(
                  panel.panelId,
                  newChannel,
                )
              }
          ...
          sc.getConnectedApps().then((connectedApps) => {
            connectedApps.forEach((app) => {
              ...
              if (state) {
                ...
                state.channelSockets.forEach((socket) =>
                  socket.emit(CHANNEL_RECEIVER_UPDATE, ur),
                )
              }
            })
          })
    ...
    // Handles channel change requests from clients, updating the app's associated channel
    socket.on(
      SAIL_CHANNEL_CHANGE,
      async function (
        ...
        session
          ...
          .then(async (resp) => {
            console.log(
              "SAIL JOIN USER CHANNEL RESPONSE: " + JSON.stringify(resp),
            )
    ...
    // Handles initial connection from an application, verifying its instance ID and setting up communication
    socket.on(
      APP_HELLO,
      async function (
        props: AppHelloArgs,
        ...
        if (fdc3Server != undefined) {
          ...
          const directoryItem = fdc3Server
            .getServerContext()
          ...
          if (appInstance != undefined && appInstance.state == State.Pending) {
            ...
            fdc3ServerInstance = fdc3Server
            fdc3ServerInstance.serverContext.setInstanceDetails(
              appInstanceId,
              appInstance,
            )
            ...
            console.error(
              "App tried to connect with invalid instance id, allowing connection anyway ",
              appInstanceId,
            )

            const shm: SailHostManifest = directoryItem[0]?.hostManifests
              ?.sail as any
    ...
    // Processes FDC3 events from applications to the Desktop Agent, such as broadcasts and context messages
    ...
    // Handles connection from a channel receiver, associating it with an app instance
    socket.on(
      CHANNEL_RECEIVER_HELLO,
      async function (
        ...
        fdc3ServerInstance = await getFdc3ServerInstance(
          sessions,
          userSessionId,
        )
        const appInstance = fdc3ServerInstance
          .getServerContext()
          .getInstanceDetails(props.instanceId)
        if (appInstance) {
    ...
    // Resolves intent requests by opening specific apps on designated channels
    socket.on(
      SAIL_INTENT_RESOLVE_ON_CHANNEL,
      function (
    ...
    // Handles disconnection events, cleaning up resources based on the type of connection that was disconnected
      ...
      if (fdc3ServerInstance) {
        if (type == SocketType.APP) {
          ...
          const details = fdc3ServerInstance
            .getServerContext()
            .getInstanceDetails(appId!)
          if (details) {
            details.channelSockets = []
            fdc3ServerInstance
              .getServerContext()
              .setInstanceDetails(appId, details)
            console.error(
              `Channel Selector Disconnect`,
              appInstanceId,
              userSessionId,
            )
          }
