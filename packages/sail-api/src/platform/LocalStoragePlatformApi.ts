 ...
 */
export interface LocalStoragePlatformApiConfig {
  /**
 ...
 * @example
 * ```typescript
 * const platformApi = new LocalStoragePlatformApi({
 *   keyPrefix: "sail_",
 *   debug: true
 * })
 *
...
export class LocalStoragePlatformApi implements ISailPlatformApi {
  ...
  constructor(config?: LocalStoragePlatformApiConfig) {
  ...
  private getItem<T>(key: string): T | null {
    try {
      ...
      if (this.debug) {
        console.error(`[LocalStoragePlatformApi] Error reading ${key}:`, error)
      }
      return null
    }
  }
  ...
  private setItem(key: string, value: unknown): void {
    try {
      ...
      if (this.debug) {
        console.error(`[LocalStoragePlatformApi] Error writing ${key}:`, error)
      }
      throw error
    }
  }
