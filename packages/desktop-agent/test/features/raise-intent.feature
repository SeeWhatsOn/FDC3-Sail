Feature: Raising Intents
  ...
  Background:
    ...
    And "appId: uniqueIntentApp, instanceId: c1" registers an intent listener for "uniqueIntent"
  ...
  Scenario: Context Not Handled By App
    When "appId: App1, instanceId: a1" raises an intent for "ViewChart" with contextType "fdc3.instrument" on app "appId: listenerApp, instanceId: b1"
  ...
  Scenario: Raising an intent that should auto-resolve (only one option)
    And "appId: App1, instanceId: a1" raises an intent for "uniqueIntent" with contextType "fdc3.instrument"
    Then messaging will have outgoing posts
  ...
  Scenario: Raising an Intent to a Non-Existent App
    And "appId: App1, instanceId: a1" raises an intent for "ViewPortfolio" with contextType "fdc3.portfolio" on app "completelyMadeUp"
    Then messaging will have outgoing posts
  ...
  Scenario: Raising An Intent To A Non-Existent App Instance
    When "appId: App1, instanceId: a1" raises an intent for "ViewPortfolio" with contextType "fdc3.portfolio" on app "portfolioApp/unknownInstance"
    Then messaging will have outgoing posts
      | msg.type            | msg.payload.error         | to.instanceId |
      | raiseIntentResponse | TargetInstanceUnavailable | a1            |

  Scenario: Raising An Intent To A Running App instance by instanceId
    When "appId: App1, instanceId: a1" raises an intent for "ViewChart" with contextType "fdc3.portfolio" on app "appId: listenerApp, instanceId: b1"
    Then messaging will have outgoing posts
      | msg.matches_type    | msg.payload.context.type | msg.payload.intent | msg.payload.originatingApp.appId | msg.payload.originatingApp.instanceId | msg.payload.intentResolution.intent | to.instanceId | to.appId    | msg.payload.intentResolution.source.appId |
  ...
  Scenario: Raising An Intent To A Non-Running App
    When "appId: App1, instanceId: a1" raises an intent for "ViewPortfolio" with contextType "fdc3.portfolio" on app "portfolioApp"
    And "uuid-0" sends validate
    And "appId: portfolioApp, instanceId: 0" registers an intent listener for "ViewPortfolio"
    Then messaging will have outgoing posts
  ...
  Scenario: Raising An Intent That Applies to A Non-Running But Uniquely Identifiable App
    When "appId: App1, instanceId: a1" raises an intent for "ViewInstrument" with contextType "fdc3.instrument"
    And "uuid-0" sends validate
    And "appId: portfolioApp, instanceId: 0" registers an intent listener for "ViewInstrument"
    Then messaging will have outgoing posts
      | msg.matches_type          | msg.payload.intent | to.instanceId | to.appId     | msg.payload.context.type |
      | addIntentListenerResponse | {null}             |             0 | portfolioApp | {null}                   |
  ...
  Scenario: Raising an Intent to a Non-Existent App Instance
    And "appId: App1, instanceId: a1" raises an intent for "ViewPortfolio" with contextType "fdc3.portfolio" on app "appId: unusedApp, instanceId: u1"
    Then messaging will have outgoing posts
      | msg.type            | msg.payload.error         | to.instanceId | to.appId |
      | raiseIntentResponse | TargetInstanceUnavailable | a1            | App1     |

  Scenario: Raising an Intent to an invalid App Instance
    And "appId: App1, instanceId: a1" raises an intent for "ViewPortfolio" with contextType "fdc3.portfolio" on an invalid app instance
    Then messaging will have outgoing posts
      | msg.type            | msg.payload.error    | to.instanceId | to.appId |
      | raiseIntentResponse | TargetAppUnavailable | a1            | App1     |

  Scenario: Raising An Intent To A Non-Running App without A Context Type in the listener
    When "appId: App1, instanceId: a1" raises an intent for "UpdatePortfolio" with contextType "fdc3.portfolio" on app "portfolioApp"
    And "uuid-0" sends validate
  ...
  Scenario: User Must Choose An Intent using The Intent Resolver
    When "appId: App1, instanceId: a1" raises an intent for "ViewChart" with contextType "fdc3.portfolio"
    Then messaging will have outgoing posts
  ...
  Scenario: Dynamic registrations are displayed in the app resolver
    When "appId: App2, instanceId: a2" registers an intent listener for "ViewChart" with contextType "fdc3.portfolio"
    When "appId: App1, instanceId: a1" raises an intent for "ViewChart" with contextType "fdc3.portfolio"
    Then messaging will have outgoing posts
      | msg.type            | msg.payload.appIntent.intent.name | msg.payload.appIntent.intent.displayName | to.instanceId | to.appId |
      | raiseIntentResponse | ViewChart                         | ViewChart                                | a1            | App1     |
  ...
  Scenario: Raising An Invalid Intent to the server (no instance)
    When "appId: App1, instanceId: a1" raises an intent for "ViewChart" with contextType "fdc3.portfolio" on app "appId: listenerApp, instanceId: z1"
    Then messaging will have outgoing posts
      | msg.payload.error         | msg.type            |
      | TargetInstanceUnavailable | raiseIntentResponse |

  Scenario: Raising An Invalid Intent (no app)
    When "appId: App1, instanceId: a1" raises an intent for "ViewChart" with contextType "fdc3.portfolio" on app "nonExistentApp"
    Then messaging will have outgoing posts
      | msg.payload.error    | msg.type            |
      | TargetAppUnavailable | raiseIntentResponse |

  Scenario: Raising An Invalid Intent (non existent intent)
    When "appId: App1, instanceId: a1" raises an intent for "nonExistentIntent" with contextType "fdc3.portfolio"
    Then messaging will have outgoing posts
      | msg.payload.error | msg.type            |
      | NoAppsFound       | raiseIntentResponse |

  Scenario: Raising An Invalid Intent (non existent intent but valid app)
    When "appId: App1, instanceId: a1" raises an intent for "nonExistentIntent" with contextType "fdc3.portfolio" on app "appId: listenerApp, instanceId: b1"
    Then messaging will have outgoing posts
      | msg.payload.error | msg.type            |
      | NoAppsFound       | raiseIntentResponse |
