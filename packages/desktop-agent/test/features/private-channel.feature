Feature: Relaying Private Channel Broadcast messages
  ...
  Scenario: Creating a new private channel
    When "appId: App2, instanceId: a1" creates a private channel
  ...
  Scenario: Broadcast message to no-one
    When "appId: App1, instanceId: a1" broadcasts "fdc3.instrument" on "{channel1Id}"
    Then messaging will have outgoing posts
      | msg.matches_type  |
      | broadcastResponse |

  Scenario: Broadcast message sent to one listener
    When "appId: App2, instanceId: a2" adds a context listener on "{channel1Id}" with type "fdc3.instrument"
    And "appId: App1, instanceId: a1" broadcasts "fdc3.instrument" on "{channel1Id}"
    Then messaging will have outgoing posts
  ...
  Scenario: Event Listener created for addContextListener and unsubscribe
    When "appId: App2, instanceId: a2" adds an "addContextListener" event listener on "{channel1Id}"
    ...
    And "appId: App1, instanceId: a1" removes context listener with id "uuid12"
    Then messaging will have outgoing posts
  ...
  Scenario: Disconnecting from a channel sends unsubscribe and disconnect messages
    ...
    And "appId: App1, instanceId: a1" adds a context listener on "{channel1Id}" with type "fdc3.instrument"
    And "App2/a2" adds an "unsubscribe" event listener on "{channel1Id}"
    And "appId: App1, instanceId: a1" disconnects from private channel "{channel1Id}"
    Then messaging will have outgoing posts
  ...
  Scenario: addContextListener Event Listener add and removed, shouldn't fire when addContextListener called.
    When "appId: App2, instanceId: a2" adds an "addContextListener" event listener on "{channel1Id}"
    And "appId: App2, instanceId: a2" removes event listener "uuid6"
    And "appId: App1, instanceId: a1" adds a context listener on "{channel1Id}" with type "fdc3.instrument"
    Then messaging will have outgoing posts
      | msg.matches_type                               | to.appId | to.instanceId | msg.payload.privateChannelId | msg.payload.contextType | msg.payload.listenerUUID |
      | privateChannelAddEventListenerResponse         | App2     | a2            | {null}                       | {null}                  | uuid6                    |
  ...
  Scenario: I can't register an app channel with the same ID as a private channel
    When "appId: App2, instanceId: a2" creates or gets an app channel called "{channel1Id}"
    Then messaging will have outgoing posts
  ...
  Scenario: Subscribe to a non-existent channel
    When "appId: App2, instanceId: a2" adds a context listener on "IDontExist" with type "fdc3.instrument"
    Then messaging will have outgoing posts
      | msg.type                   | to.appId | to.instanceId | msg.payload.error |
      | addContextListenerResponse | App2     | a2            | NoChannelFound    |

  Scenario: Can't unsubscribe an unconnected listener
    When "appId: App2, instanceId: a2" adds a context listener on "{channelId}" with type "fdc3.instrument"
  ...
  Scenario: Can't unsubscribe an someone else's listener
    When "appId: App2, instanceId: a2" adds a context listener on "{channelId}" with type "fdc3.instrument"
    And "appId: App1, instanceId: a1" removes context listener with id "uuid6"
    Then messaging will have outgoing posts
      | msg.type                           | to.appId | to.instanceId | msg.payload.error |
