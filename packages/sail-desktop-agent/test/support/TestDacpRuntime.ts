...
import { AbstractDacpRuntime } from "../../src/AbstractDacpRuntime"
import { Directory } from "../../src/app-directory/DirectoryInterface"
...
import { ChannelState } from "../../src/DacpRuntime"
...
export class TestDacpRuntime extends AbstractDacpRuntime {
...
export function createTestDacpRuntime(
  cw: CustomWorld,
  channels: ChannelState[],
  directory: Directory,
  heartbeats: boolean,
  ...
  // Open-with-context scenarios wait ~100ms then add a listener; timeout must
  // be comfortably larger. The no-listener scenario waits 3100ms for AppTimeout.
  handlers.push(new OpenHandler(2000))

  if (heartbeats) {
    // Feature waits 500ms between acks. Disconnect/dead must be > ack gap, and
    // dead must fire strictly before the 3000ms "no response" wait ends
    // (HeartbeatHandler uses `>` not `>=` on the dead threshold).
    handlers.push(new HeartbeatHandler(500, 1000, 2000))
  }

  return new TestDacpRuntime(cw, handlers, channels, directory)
}
