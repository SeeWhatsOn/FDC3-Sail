...
| Context broadcast (user channel) | `test/features/context/broadcast.feature` — Broadcast Event Includes OriginatingApp Metadata; Broadcast Is A No-Op When Not Joined To A User Channel | partial | Broadcast behavior covered, but toolbox `UCContextMetadataOnBroadcast` requires exact ContextMetadata `source` + `timestamp` assertions and WCP delivery evidence; owner: **context-metadata-conformance-bdd** |
