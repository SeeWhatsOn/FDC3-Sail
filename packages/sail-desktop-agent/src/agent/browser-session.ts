...
const collapsedBrowserAgentConnections = new WeakMap<DesktopAgent, BrowserAppConnection>()
...
export function markCollapsedBrowserDesktopAgent(
  ...
  if (browserAppConnection) {
    collapsedBrowserAgentConnections.set(desktopAgent, browserAppConnection)
  }
  if (intentResolverUI) {
...
export function createBrowserHostControllers(
  ...
  const channels: BrowserChannelsController = {
    ...
    changeAppChannel: (instanceId, channelId) => {
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        const cleanup = () => {
          ...
          browserAppConnection.off("channelChanged", handleChannelChanged)
        }

        browserAppConnection.on("channelChanged", handleChannelChanged)
    ...
    onAppChannelChange: listener => {
      ...
      browserAppConnection.on("channelChanged", handler)
      return () => {
        browserAppConnection.off("channelChanged", handler)
      }
    },
  }
  ...
  const apps: BrowserAppsController = {
    ...
    onConnect: listener => {
      ...
      return () => {
        browserAppConnection.off("appConnected", listener)
      }
    },
    onDisconnect: listener => {
      browserAppConnection.on("appDisconnected", listener)
      return () => {
        browserAppConnection.off("appDisconnected", listener)
      }
    },
    onHandshakeFailure: listener => {
      ...
      browserAppConnection.on("handshakeFailed", handler)
      return () => {
        browserAppConnection.off("handshakeFailed", handler)
      }
    },
  }
