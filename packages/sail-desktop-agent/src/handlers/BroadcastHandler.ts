...
export class BroadcastHandler implements MessageHandler {
  ...
  async accept(msg: AppRequestMessage, sc: DacpRuntime, uuid: InstanceID) {
      ...
      switch (msg.type as string | null) {
        ...
        case "getOrCreateChannelRequest":
          return this.handleGetOrCreateRequest(
            msg as GetOrCreateChannelRequest,
            sc,
            from,
          )
        ...
        case "getCurrentContextRequest":
          return this.handleGetCurrentContextRequest(
            msg as GetCurrentContextRequest,
            sc,
            from,
          )
  ...
  handleGetCurrentContextRequest(
    ...
    if (channel) {
      const context = type
        ? (channel.context.find((c) => c.type == type) ?? null)
  ...
  handleBroadcastRequest(
    ...
    const matchingListeners = sc
      ...
      .filter(
        (r) =>
          r.contextType == null || r.contextType == arg0.payload.context.type,
      )
  ...
  async handleEvent(e: DacpRuntimeEvent, sc: DacpRuntime): Promise<void> {
    if (e.type === "privateChannelDisconnect") {
      const event = e as PrivateChannelDisconnectDacpRuntimeEvent
      ...
      const event = e as ChannelChangedDacpRuntimeEvent
