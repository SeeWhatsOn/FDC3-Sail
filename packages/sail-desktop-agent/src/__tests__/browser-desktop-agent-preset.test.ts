...
import { getBrowserDesktopAgentSession, isBrowserDesktopAgent } from "../agent/browser-session"
...
import { retrieveAllApps } from "../app-directory/app-directory-queries"
...
type BrowserDesktopAgentFactory = (options?: {
  ...
  appConnectionOptions?: { intentResolutionTimeout?: number }
...
describe("createBrowserDesktopAgent top-level preset", () => {
  ...
  it("returns a DesktopAgent with coupled browser edge lifecycle", () => {
    ...
    expect(getBrowserDesktopAgentSession(desktopAgent).browserAppConnection).toBeDefined()
  ...
  it("exposes framework-neutral intent resolver UI methods from the browser preset", async () => {
    ...
    const resolutionPromise = session.browserAppConnection.requestIntentResolution({
  ...
  it("cancels through intentResolverUI before the WCP timeout fires", async () => {
    ...
    const createBrowserDesktopAgent = requireBrowserDesktopAgentFactory()
    const desktopAgent = createBrowserDesktopAgent({
      appConnectionOptions: { intentResolutionTimeout: 20 },
    })
    activeAgents.push(desktopAgent)
...
describe("desktopAgent.intentResolver canonical host controller", () => {
  ...
  it("cancels pending resolution when cancel is called", async () => {
    ...
    const createBrowserDesktopAgent = requireBrowserDesktopAgentFactory()
    const desktopAgent = createBrowserDesktopAgent({
      appConnectionOptions: { intentResolutionTimeout: 60_000 },
    })
    activeAgents.push(desktopAgent)
  ...
  it("stops delivering requests to a listener after onRequest unsubscribe", async () => {
    ...
    const firstResolution = session.browserAppConnection.requestIntentResolution({
    ...
    const secondResolution = session.browserAppConnection.requestIntentResolution({
...
describe("browser host controller composition", () => {
  ...
  it("exports createBrowserHostControllers for manual DesktopAgent + BrowserAppConnection composition", () => {
